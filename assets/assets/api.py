...
def init_machine(request):
    ...
    phy_page = request.GET.get('phy_page', '')
    val_page = request.GET.get('val_page', '')
    # print all, type, id, env, system, 'phy_page: ', phy_page, 'val_page: ', val_page
    oper = OperaMachine()
    res = oper._get_info(all, type, id, env, system, ip, phy_page, val_page)
    #print 20 * '*'
    #print res
    return JsonResponse(res)
